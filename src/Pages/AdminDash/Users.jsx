...
const Users = () => {
  ...
  const handleAdmin = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axiosSecure.patch(`/user/admin/${id}`, { role: "admin" }).then((res) => {
  ...
  const handleTeacher = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axiosSecure.patch(`/user/admin/${id}`, { role: "teacher" }).then((res) => {
          if (res.data.modifiedCount > 0) {
            refetch();
            Swal.fire({
              title: "Teacher",
              text: "User Now Teacher.",
              icon: "success",
            });
          }
        });
                ...
                <tr key={item._id}>
                  ...
                  <th>
                    <button
                      ...
                      onClick={() => handleTeacher(item._id)}
                    >
                      Make Teacher
                    </button>
                  </th>
